...
pca.to_csv("pca_unprocessed_mito.csv",index=False)
...
coordinates_no_outliers['PC1_jittered'] = coordinates_no_outliers['PC1'] + np.random.normal(0, jitter_amount, len(coordinates_no_outliers))
coordinates_no_outliers['PC2_jittered'] = coordinates_no_outliers['PC2'] + np.random.normal(0, jitter_amount, len(coordinates_no_outliers))
coordinates_no_outliers['PC3_jittered'] = coordinates_no_outliers['PC3'] + np.random.normal(0, jitter_amount, len(coordinates_no_outliers))
...
plt.savefig('PCA_EMAGEN_mito.png', dpi=300, bbox_inches='tight')
plt.show()
...
for site in unique_sites:
    subset = coordinates_no_outliers[coordinates_no_outliers['site'] == site]
    axes[0].scatter(subset['PC1_jittered'], subset['PC2_jittered'], s=15, label=site, alpha=0.7, color=colors[site])

axes[0].set_xlabel(f'PC1_jittered ({explained_variance_ratio[0]*100:.2f}% )')
axes[0].set_ylabel(f'PC2_jittered ({explained_variance_ratio[1]*100:.2f}% )')
#axes[0].set_title("A")

# Plot PC3 vs PC2 in the second subplot
for site in unique_sites:
    subset = coordinates_no_outliers[coordinates_no_outliers['site'] == site]
    axes[1].scatter(subset['PC2_jittered'], subset['PC3_jittered'], s=15, label=site, alpha=0.7, color=colors[site])

axes[1].set_xlabel(f'PC2_jittered ({explained_variance_ratio[1]*100:.2f}% )')
axes[1].set_ylabel(f'PC3_jittered ({explained_variance_ratio[2]*100:.2f}% )')
#axes[1].set_title("B")
...
plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig('PCA_EMAGEN_mito_jittered.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('PCA_EMAGEN_pvproportion_mito.png', dpi=300, bbox_inches='tight')
plt.show()


# jittered -  Set up a plot for the first 2 principal components without outliers
fig, axes = plt.subplots(1, 2, figsize=(12, 4.0), sharey=False)
...
#Plot PC1 vs PC2 in the first subplot
sc1 = axes[0].scatter(coordinates_no_outliers['PC1_jittered'], coordinates_no_outliers['PC2_jittered'], 
                      c=coordinates_no_outliers['pv_proportion'], 
                      cmap=blue_cmap, s=15, alpha=0.7,vmin=vmin, vmax=vmax)
axes[0].set_xlabel(f'PC1_jittered ({explained_variance_ratio[0]*100:.2f}% )')
axes[0].set_ylabel(f'PC2_jittered ({explained_variance_ratio[1]*100:.2f}% )')
...
# Plot PC3 vs PC2 in the second subplot
sc2 = axes[1].scatter(coordinates_no_outliers['PC2_jittered'], coordinates_no_outliers['PC3_jittered'], 
                      c=coordinates_no_outliers['pv_proportion'], 
                      cmap=blue_cmap, s=15, alpha=0.7,vmin=vmin, vmax=vmax)
axes[1].set_xlabel(f'PC2_jittered ({explained_variance_ratio[1]*100:.2f}% )')
axes[1].set_ylabel(f'PC3_jittered ({explained_variance_ratio[2]*100:.2f}% )')
...
plt.tight_layout(rect=[0, 0, 0.7, 1])
plt.savefig('PCA_EMAGEN_pvproportion_mito_jittered.png', dpi=300, bbox_inches='tight')
plt.show()
