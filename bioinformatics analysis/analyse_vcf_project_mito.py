...
pca.to_csv("pca_unprocessed_projects_mito.csv",index=False)
...
plt.savefig('PCA_project_mito.png', dpi=300, bbox_inches='tight')
plt.show()
...
for site in unique_sites:
    subset = coordinates_no_outliers[coordinates_no_outliers['site'] == site]
    axes[0].scatter(subset['PC1_jittered'], subset['PC2_jittered'], s=15, label=site, alpha=0.5, color=distinct_colors.get(site, '#808080'))
...
for site in unique_sites:
    subset = coordinates_no_outliers[coordinates_no_outliers['site'] == site]
    axes[1].scatter(subset['PC2_jittered'], subset['PC3_jittered'], s=15, label=site, alpha=0.5, color=distinct_colors.get(site, '#808080'))
...
# Place a single legend outside the plots
handles, labels = axes[1].get_legend_handles_labels()
fig.legend(handles, labels, title='Project', markerscale=2,bbox_to_anchor=(0.879, 0.55), loc='center left')

plt.tight_layout(rect=[0, 0, 0.9, 1])
plt.savefig('PCA_project_mito_jittered.png', dpi=300, bbox_inches='tight')
plt.show()
